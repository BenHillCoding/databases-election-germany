-- German 2021 Federal Election Seat Allocation using Views
...
DROP MATERIALIZED VIEW IF EXISTS mv_national_votes_21 CASCADE;

CREATE MATERIALIZED VIEW mv_national_votes_21 AS
SELECT
  p.id as partei_id,
  p.name as partei_name,
  SUM(z.anzahl) as total_votes,
  ROUND((SUM(z.anzahl)::DECIMAL / (SELECT SUM(anzahl) FROM zweitstimme JOIN wahlkreisergebnis ON zweitstimme.wahlkreisergebnis_id = wahlkreisergebnis.id WHERE gueltig = true AND wahlkreisergebnis.wahl_id = 20))::NUMERIC * 100, 2) as vote_share_percent
FROM zweitstimme z
JOIN partei p ON z.partei_id = p.id
JOIN wahlkreisergebnis we ON we.id = z.wahlkreisergebnis_id
WHERE z.gueltig = true AND we.wahl_id = 20
...
DROP MATERIALIZED VIEW IF EXISTS mv_national_direct_mandates_21 CASCADE;

CREATE MATERIALIZED VIEW mv_national_direct_mandates_21 AS
SELECT
  p.id as partei_id,
  p.name as partei_name,
...
DROP MATERIALIZED VIEW IF EXISTS mv_partei_wahlkreisgewinne_21 CASCADE;

CREATE MATERIALIZED VIEW mv_partei_wahlkreisgewinne_21 AS
WITH stimmen_pro_kandidat AS (
    ...
    WHERE e.gueltig = true AND wke.wahl_id = 20
...
DROP MATERIALIZED VIEW IF EXISTS mv_qualifying_parties_21 CASCADE;

CREATE MATERIALIZED VIEW mv_qualifying_parties_21 AS
SELECT
  nv.partei_id,
  nv.partei_name,
  nv.total_votes,
...
FROM mv_national_votes_21 nv
LEFT JOIN mv_national_direct_mandates_21 ndm ON nv.partei_id = ndm.partei_id
JOIN partei p ON nv.partei_id = p.id
LEFT JOIN mv_partei_wahlkreisgewinne_21 wkg ON p.id = wkg.partei_id
...
DROP MATERIALIZED VIEW IF EXISTS mv_party_bundesland_votes_21 CASCADE;

CREATE MATERIALIZED VIEW mv_party_bundesland_votes_21 AS
SELECT
  p.id as partei_id,
  p.name as partei_name,
  b.id as bundesland_id,
  b.name as bundesland_name,
  SUM(z.anzahl) as second_votes
FROM zweitstimme z
JOIN partei p ON z.partei_id = p.id
...
WHERE z.gueltig = true
  AND p.id IN (SELECT partei_id FROM mv_qualifying_parties_21)
  AND wr.wahl_id = 20
...
DROP MATERIALIZED VIEW IF EXISTS mv_direct_mandates_21 CASCADE;

CREATE MATERIALIZED VIEW mv_direct_mandates_21 AS
SELECT
  p.id as partei_id,
  p.name as partei_name,
  b.id as bundesland_id,
...
DROP MATERIALIZED VIEW IF EXISTS mv_total_second_votes_21 CASCADE;

CREATE MATERIALIZED VIEW mv_total_second_votes_21 AS
SELECT SUM(z.anzahl) as total_votes
FROM zweitstimme z
JOIN mv_qualifying_parties_21 qp
    ON z.partei_id = qp.partei_id
JOIN wahlkreisergebnis we ON z.wahlkreisergebnis_id = we.id
WHERE gueltig = true AND we.wahl_id = 20;

-- Create materialized view to cache divisor calculation. Result: divisor, iteration in which divisor was found
DROP MATERIALIZED VIEW IF EXISTS mv_saintelague_divisor_21 CASCADE;

CREATE MATERIALIZED VIEW mv_saintelague_divisor_21 AS
WITH RECURSIVE divisor_iteration AS (
  -- Start with better initial divisor: total_votes / 630
  SELECT
    (SELECT total_votes::numeric FROM mv_total_second_votes_21) / 630.0 as divisor,
    1 as iteration
  ...
  SELECT
    CASE
      WHEN calc.seat_sum > 630 THEN (di.divisor * 1.0005)::numeric
      WHEN calc.seat_sum < 630 THEN (di.divisor * 0.9995)::numeric
      ELSE di.divisor
    END as divisor,
    di.iteration + 1
  ...
  CROSS JOIN LATERAL (
    ...
    FROM mv_national_votes_21 nv
    WHERE nv.partei_id IN (SELECT partei_id FROM mv_qualifying_parties_21)
  ) calc
...
DROP MATERIALIZED VIEW IF EXISTS mv_national_seat_allocation_21 CASCADE;

CREATE MATERIALIZED VIEW mv_national_seat_allocation_21 AS
SELECT
  nv.partei_id,
  nv.partei_name,
  nv.total_votes,
  nv.vote_share_percent,
  (nv.total_votes::numeric / (SELECT divisor FROM mv_saintelague_divisor_21)) as seat_value,
  ROUND(nv.total_votes::numeric / (SELECT divisor FROM mv_saintelague_divisor_21))::int as total_seats_allocated
FROM mv_national_votes_21 nv
WHERE nv.partei_id IN (SELECT partei_id FROM mv_qualifying_parties_21)
...
DROP MATERIALIZED VIEW IF EXISTS mv_party_bundesland_divisors_21 CASCADE;

CREATE MATERIALIZED VIEW mv_party_bundesland_divisors_21 AS
WITH party_targets AS (
  SELECT
    partei_id,
    total_seats_allocated as target_seats
  FROM mv_national_seat_allocation_21
)
SELECT
  nsa.partei_id,
  nsa.partei_name,
    ...
    ELSE (
      SELECT divisor FROM (
        WITH RECURSIVE divisor_iteration AS (
          ...
          SELECT
            (SELECT SUM(second_votes)::numeric FROM mv_party_bundesland_votes_21 pbv WHERE pbv.partei_id = nsa.partei_id) /
          ...
          SELECT
            CASE
              WHEN calc.total_seats > pt.target_seats THEN (di.divisor * 1.0005)::numeric
              WHEN calc.total_seats < pt.target_seats THEN (di.divisor * 0.9995)::numeric
              ELSE di.divisor
            END as divisor,
            di.iteration + 1
          FROM divisor_iteration di
          CROSS JOIN LATERAL (
            ...
            FROM mv_party_bundesland_votes_21 pbv
  ...
  END as divisor
FROM mv_national_seat_allocation_21 nsa
...
DROP MATERIALIZED VIEW IF EXISTS mv_bundesland_seat_distribution_21 CASCADE;

CREATE MATERIALIZED VIEW mv_bundesland_seat_distribution_21 AS
SELECT
  pb.partei_id,
  pb.partei_name,
  pb.bundesland_id,
...
FROM mv_party_bundesland_votes_21 pb
JOIN mv_national_seat_allocation_21 nsa ON pb.partei_id = nsa.partei_id
JOIN mv_party_bundesland_divisors_21 pbd ON pb.partei_id = pbd.partei_id
...
DROP MATERIALIZED VIEW IF EXISTS mv_mandate_coverage_21 CASCADE;

CREATE MATERIALIZED VIEW mv_mandate_coverage_21 AS
SELECT
...
FROM mv_bundesland_seat_distribution_21 bsd
FULL OUTER JOIN mv_direct_mandates_21 dm
...
DROP MATERIALIZED VIEW IF EXISTS mv_final_seat_distribution_21 CASCADE;

CREATE MATERIALIZED VIEW mv_final_seat_distribution_21 AS
SELECT
  partei_id,
  partei_name,
  bundesland_id,
...
DROP MATERIALIZED VIEW IF EXISTS mv_allocation_summary_21 CASCADE;

CREATE MATERIALIZED VIEW mv_allocation_summary_21 AS
SELECT
  (SELECT SUM(total_seats) FROM mv_final_seat_distribution_21) as total_seats_distributed,
  (SELECT COUNT(DISTINCT partei_id) FROM mv_final_seat_distribution_21) as parties_represented,
  (SELECT COUNT(DISTINCT bundesland_id) FROM mv_final_seat_distribution_21) as bundeslands_affected,
  (SELECT COUNT(*) FROM mv_qualifying_parties_21) as parties_qualifying,
  (SELECT total_votes FROM mv_total_second_votes_21) as total_second_votes;
...
DROP MATERIALIZED VIEW IF EXISTS mv_national_summary_21 CASCADE;

CREATE MATERIALIZED VIEW mv_national_summary_21 AS
SELECT
  qp.partei_id,
  qp.partei_name,
  qp.total_votes,
...
FROM mv_qualifying_parties_21 qp
