...
DROP MATERIALIZED VIEW IF EXISTS mv_national_votes_25 CASCADE;

CREATE MATERIALIZED VIEW mv_national_votes_25 AS
SELECT
  p.id as partei_id,
...
DROP MATERIALIZED VIEW IF EXISTS mv_national_direct_mandates_25 CASCADE;

CREATE MATERIALIZED VIEW mv_national_direct_mandates_25 AS
SELECT
  p.id as partei_id,
  p.name as partei_name,
...
DROP MATERIALIZED VIEW IF EXISTS mv_partei_wahlkreisgewinne_25 CASCADE;

CREATE MATERIALIZED VIEW mv_partei_wahlkreisgewinne_25 AS
...
DROP MATERIALIZED VIEW IF EXISTS mv_qualifying_parties_25 CASCADE;

CREATE MATERIALIZED VIEW mv_qualifying_parties_25 AS
SELECT
  nv.partei_id,
  nv.partei_name,
  nv.partei_kuerzel,
...
FROM mv_national_votes_25 nv
LEFT JOIN mv_national_direct_mandates_25 ndm ON nv.partei_id = ndm.partei_id
JOIN partei p ON nv.partei_id = p.id
LEFT JOIN mv_partei_wahlkreisgewinne_25 wkg ON p.id = wkg.partei_id
...
DROP MATERIALIZED VIEW IF EXISTS mv_party_bundesland_votes_25 CASCADE;

CREATE MATERIALIZED VIEW mv_party_bundesland_votes_25 AS
SELECT
  p.id as partei_id,
  p.name as partei_name,
...
WHERE z.gueltig = true
  AND p.id IN (SELECT partei_id FROM mv_qualifying_parties_25)
  AND wr.wahl_id = 21
...
DROP MATERIALIZED VIEW IF EXISTS mv_direct_mandates_25 CASCADE;

CREATE MATERIALIZED VIEW mv_direct_mandates_25 AS
SELECT
  p.id as partei_id,
  p.name as partei_name,
  b.id as bundesland_id,
...
DROP MATERIALIZED VIEW IF EXISTS mv_total_second_votes_25 CASCADE;

CREATE MATERIALIZED VIEW mv_total_second_votes_25 AS
SELECT COUNT(z.id) as total_votes
FROM zweitstimme z
JOIN mv_qualifying_parties_25 qp
...
DROP MATERIALIZED VIEW IF EXISTS mv_saintelague_divisor_25 CASCADE;

CREATE MATERIALIZED VIEW mv_saintelague_divisor_25 AS
WITH RECURSIVE divisor_iteration AS (
  -- Start with better initial divisor: total_votes / 630
  SELECT
    (SELECT total_votes::numeric FROM mv_total_second_votes_25) / 630.0 as divisor,
    1 as iteration
  ...
  CROSS JOIN LATERAL (
    ...
    FROM mv_national_votes_25 nv
    WHERE nv.partei_id IN (SELECT partei_id FROM mv_qualifying_parties_25)
  ) calc
...
DROP MATERIALIZED VIEW IF EXISTS mv_national_seat_allocation_25 CASCADE;

CREATE MATERIALIZED VIEW mv_national_seat_allocation_25 AS
SELECT
  nv.partei_id,
  nv.partei_name,
  nv.total_votes,
  nv.vote_share_percent,
  (nv.total_votes::numeric / (SELECT divisor FROM mv_saintelague_divisor_25)) as seat_value,
  ROUND(nv.total_votes::numeric / (SELECT divisor FROM mv_saintelague_divisor_25))::int as total_seats_allocated
FROM mv_national_votes_25 nv
WHERE nv.partei_id IN (SELECT partei_id FROM mv_qualifying_parties_25)
...
DROP MATERIALIZED VIEW IF EXISTS mv_party_bundesland_divisors_25 CASCADE;

CREATE MATERIALIZED VIEW mv_party_bundesland_divisors_25 AS
WITH party_targets AS (
  SELECT
    partei_id,
    total_seats_allocated as target_seats
  FROM mv_national_seat_allocation_25
)
SELECT
  nsa.partei_id,
  nsa.partei_name,
    ...
    ELSE (
      SELECT divisor FROM (
        WITH RECURSIVE divisor_iteration AS (
          ...
          SELECT
            (SELECT SUM(second_votes)::numeric FROM mv_party_bundesland_votes_25 pbv WHERE pbv.partei_id = nsa.partei_id) /
          ...
          CROSS JOIN LATERAL (
            ...
            FROM mv_party_bundesland_votes_25 pbv
  ...
  END as divisor
FROM mv_national_seat_allocation_25 nsa
...
DROP MATERIALIZED VIEW IF EXISTS mv_bundesland_seat_distribution_25 CASCADE;

CREATE MATERIALIZED VIEW mv_bundesland_seat_distribution_25 AS
SELECT
  pb.partei_id,
  pb.partei_name,
  pb.bundesland_id,
...
FROM mv_party_bundesland_votes_25 pb
JOIN mv_national_seat_allocation_25 nsa ON pb.partei_id = nsa.partei_id
JOIN mv_party_bundesland_divisors_25 pbd ON pb.partei_id = pbd.partei_id
...
DROP MATERIALIZED VIEW IF EXISTS mv_mandate_coverage_25 CASCADE;

CREATE MATERIALIZED VIEW mv_mandate_coverage_25 AS
SELECT
...
FROM mv_bundesland_seat_distribution_25 bsd
FULL OUTER JOIN mv_direct_mandates_25 dm
...
DROP MATERIALIZED VIEW IF EXISTS mv_final_seat_distribution_25 CASCADE;

CREATE MATERIALIZED VIEW mv_final_seat_distribution_25 AS
SELECT
  partei_id,
  partei_name,
  bundesland_id,
...
DROP MATERIALIZED VIEW IF EXISTS mv_allocation_summary_25 CASCADE;

CREATE MATERIALIZED VIEW mv_allocation_summary_25 AS
SELECT
  (SELECT SUM(total_seats) FROM mv_final_seat_distribution_25) as total_seats_distributed,
  (SELECT COUNT(DISTINCT partei_id) FROM mv_final_seat_distribution_25) as parties_represented,
  (SELECT COUNT(DISTINCT bundesland_id) FROM mv_final_seat_distribution_25) as bundeslands_affected,
  (SELECT COUNT(*) FROM mv_qualifying_parties_25) as parties_qualifying,
  (SELECT total_votes FROM mv_total_second_votes_25) as total_second_votes;
...
DROP MATERIALIZED VIEW IF EXISTS mv_national_summary_25 CASCADE;

CREATE MATERIALIZED VIEW mv_national_summary_25 AS
SELECT
  qp.partei_id,
  qp.partei_name,
  qp.partei_kuerzel,
